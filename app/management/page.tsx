  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="sticky top-0 left-0 right-0 h-14 flex items-center px-4 bg-card/80 backdrop-blur-md border-b border-border z-10">
        ...
        <h1 className="flex-1 text-center text-lg font-semibold text-foreground">代理后台</h1>
      </div>

      <Tabs defaultValue="management" value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="fixed bottom-0 left-0 right-0 z-10 bg-card border-t border-border shadow-lg p-4">
        ...
        <TabsContent value="dashboard" className="p-4 pb-16">
              ...
              <Link href={shortcut.link} key={shortcut.id}>
                <Card className="p-3 bg-card border-border h-full">
                  <div className="flex items-center">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-muted mr-3">
                      {shortcut.icon}
                    </div>
                    <div>
                      <h3 className="font-medium text-foreground">{shortcut.title}</h3>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">用户总数</div>
              <div className="text-xl font-bold text-foreground">{formatNumber(dashboardData.totalUsers)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">付费会员总数</div>
              <div className="text-xl font-bold text-blue-400">{formatNumber(dashboardData.paidUsers)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">今日新增</div>
              <div className="text-xl font-bold text-green-400">{formatNumber(dashboardData.newUsersToday)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">昨日新增</div>
              <div className="text-xl font-bold text-foreground">{formatNumber(dashboardData.newUsersYesterday)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">今日付款</div>
              <div className="text-xl font-bold text-green-400">{formatNumber(dashboardData.paymentsToday)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">昨日付款</div>
              <div className="text-xl font-bold text-foreground">{formatNumber(dashboardData.paymentsYesterday)}</div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">今日收入(元)</div>
              <div className="text-xl font-bold text-green-400">{formatNumber(dashboardData.todayRevenue)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">昨日收入(元)</div>
              <div className="text-xl font-bold text-foreground">{formatNumber(dashboardData.yesterdayRevenue)}</div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">累计付款笔数</div>
              <div className="text-xl font-bold text-amber-400">{formatNumber(dashboardData.totalPayments)}</div>
            </Card>
            <Card className="p-3 bg-card border-border">
              <div className="text-xs text-muted-foreground mb-1">累计收入(元)</div>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-card border-border">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-lg font-semibold text-foreground">课程收入</CardTitle>
            ...
            <Card className="bg-card border-border">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-lg font-semibold text-foreground">考试收入</CardTitle>
            ...
            <Card className="bg-card border-border">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-lg font-semibold text-foreground">会员收入</CardTitle>
          ...
          {/* 付费转化率 */}
          <Card className="p-4 bg-card border-border mb-6 mt-6">
            <h3 className="font-medium text-foreground mb-3">付费转化率</h3>
          ...
          <div className="grid gap-3">
            {managementMenus.map((menu) => (
              <Link href={menu.link} key={menu.id}>
                <Card className="p-4 bg-card border-border flex items-center">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-muted mr-3">{menu.icon}</div>
                  <div className="flex-1">
                    <h3 className="font-medium text-foreground">{menu.title}</h3>
                  </div>
                  <div className="flex items-center">
                    {menu.badge && (
